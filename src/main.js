...
const btnQuickTx = document.getElementById('btn-quick-tx');
const txModal = document.getElementById('tx-modal');
const btnCloseTxModal = document.getElementById('btn-close-modal');
const btnCancelTxModal = document.getElementById('btn-cancel-modal');
const quickTxForm = document.getElementById('quick-tx-form');
const txDateInput = document.getElementById('tx-date');

// Quick Budget Modal Elements
const btnQuickBudget = document.getElementById('btn-quick-budget');
...
const quickBudgetForm = document.getElementById('quick-budget-form');
...
function renderRecentTransactions() {
  ...
  recentTransactionsContainer.querySelectorAll('.tx-delete-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const item = e.currentTarget.closest('.tx-item');
      if (item) {
...
function renderCashflowChart() {
  const store = window.FinanceStore;
  const transactions = [...store.getTransactions()].reverse();
  const datesMap = {};
  
  for (let i = 9; i >= 0; i--) { // adjusted to show 10 points for smoother trend line matching screenshot
    const d = new Date();
  ...
  cashflowChart = new ChartClass(ctx, {
    type: 'line',
    data: {
      labels: labels,
      datasets: [
...
function setupEventListeners() {
  ...
  quickBudgetForm.addEventListener('submit', (e) => {
    ...
    window.FinanceStore.setBudget(category, limit);
    closeBudget();
    updateDashboard();
  });
}

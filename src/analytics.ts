...
// DOM elements
const budgetProgressContainer = document.getElementById('budget-progress-container') as HTMLElement;
const goalsContainer = document.getElementById('goals-container') as HTMLElement;

// Goal Modal elements
const btnAddGoal = document.getElementById('btn-add-goal') as HTMLButtonElement;
const goalModal = document.getElementById('goal-modal') as HTMLElement;
const btnCloseGoalModal = document.getElementById('btn-close-goal-modal') as HTMLButtonElement;
const btnCancelGoalModal = document.getElementById('btn-cancel-goal-modal') as HTMLButtonElement;
...
function renderCategoryChart() {
  const categorySummary = FinanceStore.getCategorySummary();
  ...
  if (categories.length === 0) {
    // Render placeholder text in canvas if no expenses
...
function renderBudgets() {
  ...
  const categoryExpenses = FinanceStore.getCategorySummary();
...
function renderGoals() {
  ...
  goalsContainer.querySelectorAll('.goal-save-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const card = (e.currentTarget as HTMLElement).closest('.goal-card') as HTMLElement;
      const id = card.getAttribute('data-id');
      const input = card.querySelector('.goal-adjust-input') as HTMLInputElement;
      if (id && input) {
        const val = parseFloat(input.value);
        if (!isNaN(val) && val >= 0) {
          FinanceStore.updateGoalProgress(id, val);
          renderAnalytics();
        }
      }
    });
